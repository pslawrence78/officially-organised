...
export function PrepTaskCard({ familyMembers, item, onStatusChange }: PrepTaskCardProps) {
  ...
  const celebrationGenerated = isCelebrationGeneratedPrepTask(task);
  ...
  return (
    <article className={`prep-task-card prep-task-card--${task.priority}${overdue ? " is-overdue" : ""}${task.status !== "open" ? " is-cleared" : ""}`}>
      ...
      <div className="prep-task-card__body"><div className="prep-task-card__badges"><Badge tone={task.priority === "critical" ? "critical" : task.priority === "important" ? "warning" : "neutral"}>{PREP_TASK_PRIORITY_LABELS[task.priority]}</Badge><Badge tone={task.status === "done" ? "success" : "neutral"}>{PREP_TASK_STATUS_LABELS[task.status]}</Badge>{celebrationGenerated ? <Badge tone="accent">Celebrations</Badge> : null}{overdue ? <Badge tone="critical">Overdue</Badge> : null}{task.blocksEvent ? <Badge tone="critical">Blocks event</Badge> : null}</div><h3>{task.title}</h3><Link to={`/events/${event.id}`}>{event.title}</Link><p><strong>Due</strong> {formatPrepDue(task.dueAt)}</p><p><strong>Owner</strong> {owners.length ? owners.join(" & ") : "Unassigned"}</p>{task.notes ? <p className="prep-task-card__notes">{task.notes}</p> : null}</div>
