...
import { getEvents, getEventsForDateRange, getFamilyMembers, getPlaces } from "../data/repositories";
...
export function WeekPage() {
  ...
  const state = useRepositoryQuery(async () => {
    const [events, allEvents, familyMembers, places] = await Promise.all([
      getEventsForDateRange(new Date(dateKeyToIsoStart(weekStart)), new Date(dateKeyToIsoStart(weekEndExclusive))), getEvents(), getFamilyMembers(), getPlaces(),
    ]);
  ...
  return <div className="page-stack">
    ...
    {data ? <>
      ...
      <section className="week-list" aria-label={`Week of ${formatWeekRange(weekStart)}`}>{days.map((day) => {
        ...
        return <section className="week-day" key={day}><header><h2>{formatLongDate(day)}</h2><span>{dayEvents.length || "-"}</span></header>{dayEvents.length ? <div className="event-list">{dayEvents.map((event) => <EventCard conflicts={conflictsForEvent(conflicts, event.id)} event={event} familyMembers={data.familyMembers} key={event.id} place={data.places.find((place) => place.id === event.placeId)} />)}</div> : <p className="week-day__empty">No events</p>}</section>;
      })}</section>
    </> : null}
  </div>;
}
