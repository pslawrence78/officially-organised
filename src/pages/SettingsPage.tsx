...
export function SettingsPage() {
  const [phrase, setPhrase] = useState(""); const [resetting, setResetting] = useState(false); const [resetDone, setResetDone] = useState(false);
  const state = useRepositoryQuery(async () => { const [household, counts] = await Promise.all([getHousehold(), getLocalDataSummary()]); return { household, counts }; }, [resetDone]);
  const reset = async () => { if (phrase !== RESET_PHRASE) return; setResetting(true); await resetLocalDataAndReseed(); setPhrase(""); setResetDone(true); setResetting(false); };
  return <div className="page-stack">
    ...
    {state.data ? <section className="settings-card"><dl><div><dt>Household</dt><dd>{state.data.household?.name ?? "Not available"}</dd></div><div><dt>Timezone</dt><dd>{state.data.household?.timezone ?? "Not available"}</dd></div><div><dt>Local records</dt><dd>{Object.values(state.data.counts).reduce((sum, count) => sum + count, 0)}</dd></div><div><dt>Database version</dt><dd>{databaseMetadata.schemaVersion}</dd></div><div><dt>Backup schema</dt><dd><code>{EXPORT_DATA_SCHEMA}</code></dd></div></dl></section> : null}
