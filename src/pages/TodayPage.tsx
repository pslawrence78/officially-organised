...
export function TodayPage() {
  ...
  return (
    <div className="page-stack">
      ...
      {celebrationReadiness.length ? <section className="section-block"><div className="section-heading"><div><p className="eyebrow">Celebration readiness</p><h2>Today and tomorrow only</h2></div><Link className="back-link" to="/celebrations">Open Gifts &amp; Celebrations</Link></div><div className="celebration-issue-list">{celebrationReadiness.map((summary) => <article className={`celebration-issue-card celebration-issue-card--${summary.level === "overdue" ? "critical" : "warning"}`} key={summary.occasionId}><div className="celebration-issue-card__top"><div className="event-detail__badges"><CelebrationReadinessBadge level={summary.level} /><Badge tone="accent">{summary.occasionTitle}</Badge></div>{summary.occasionDate ? <small>{summary.occasionDate}</small> : null}</div><strong>{summary.issues[0]?.message ?? "Gift or celebration work needs attention."}</strong></article>)}</div></section> : null}
