...
export function CarPage() {
  ...
  return <div className="page-stack"><PageHeader eyebrow="Shared wheels" title="Car">See who needs the family car, when it is needed, and whether it has already been allocated.</PageHeader>{state.loading ? <LoadingState label="Gathering car needs..." /> : null}{state.error ? <ErrorState /> : null}{data ? <><section className="car-summary-grid"><article><strong>{summary.today}</strong><span>today</span></article><article><strong>{summary.required}</strong><span>required</span></article><article><strong>{summary.maybe}</strong><span>maybe</span></article></section><label className="prep-owner-filter"><span>Show</span><select onChange={(event) => setStatusFilter(event.target.value)} value={statusFilter}><option value="all">Required and maybe</option><option value="required">Required only</option><option value="maybe">Maybe only</option></select></label>{summary.active.length === 0 ? <section className="empty-panel"><span className="empty-panel__icon"><Icon name="car" /></span><h2>No upcoming car needs</h2><p>Mark the family car as required or maybe needed inside an event.</p></section> : groups.map(({ group, items }) => items.length ? <CarGroup familyMembers={data.familyMembers} group={group} items={items} key={group} /> : null)}</> : null}</div>;
}
